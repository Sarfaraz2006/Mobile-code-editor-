...
<script>
  ...
  window.__setEditorValue = function(v){ if (v !== editor.getValue()) { var c = editor.getCursor(); lastSent = v; editor.setValue(v); editor.setCursor(c); } };
  window.__setEditorMode = function(m){ editor.setOption('mode', m); };
  window.__doAction = function(action){
  ...
  window.__insertText = function(t){ editor.replaceSelection(t); editor.focus(); };
  post({ type: 'cc-ready' });
</script>
</body>
</html>`;
